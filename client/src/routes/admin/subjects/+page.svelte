    ...
    <div class="overflow-x-auto px-6 pb-6 pt-4">
      {#if Array.isArray(data.subjects) && data.subjects.length > 0}
        <table class="w-full min-w-[900px] text-sm text-gray-800">
          ...
          >
            <tr>
              {#each ["#", "Code", "Name", "Actions"] as header}
                <th
          ...
          <tbody class="bg-white divide-y divide-gray-100">
            {#each Array.isArray(data.subjects) ? data.subjects : [data.subjects] as subject, index}
              <tr class="group hover:bg-gray-50 transition">
                ...
                <td class="px-4 py-3 whitespace-nowrap">
                  <div class="flex items-center gap-2">
                    <button
                      onclick={() => editRecord(subject?.id.toString())}
